...
class App : Application() {
    ...
    private val orientationCallbacks =
    ...
    override fun onCreate() {
        ...
        registerActivityLifecycleCallbacks(orientationCallbacks)
        ...
        val existingLocales = AppCompatDelegate.getApplicationLocales()
        val selectedLanguage = currentLocale.toLanguageTag()
        val currentLanguage = existingLocales[0]?.toLanguageTag().orEmpty()
        if (selectedLanguage.isNotBlank() && selectedLanguage != currentLanguage) {
            val appLocale = LocaleListCompat.create(currentLocale)
            AppCompatDelegate.setApplicationLocales(appLocale)
        }

        GlobalScope.launch(Dispatchers.IO) {
            launch(Dispatchers.IO) { LspPersistence.restoreServers() }
