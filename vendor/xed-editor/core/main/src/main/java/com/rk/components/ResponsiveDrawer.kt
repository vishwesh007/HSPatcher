...
fun ResponsiveDrawer(
    ...
    var hasOpenedDrawer by remember(isPermanentDrawer) { mutableStateOf(isPermanentDrawer) }
    LaunchedEffect(drawerState.currentValue, drawerState.targetValue, isPermanentDrawer) {
        if (isPermanentDrawer || drawerState.currentValue != DrawerValue.Closed || drawerState.targetValue != DrawerValue.Closed) {
