...
open class EditorTab(override var file: FileObject, var projectRoot: FileObject?, val viewModel: MainViewModel) :
    ...
    private data class PendingSessionRestore(
    ...
    private var pendingSessionRestore: PendingSessionRestore? = null
    ...
    fun ensureInitialized() {
        if (loadJob != null || editorState.content != null || editorState.loadFailed != null) {
            return
        }

        loadJob = scope.launch {
            if (!file.exists() || !file.canRead()) {
                editorState.isLoadingContent = false
                editorState.loadFailed = "File is no longer available."
                return@launch
            }

            projectRoot = projectRoot ?: file.getParentFile()
            editorState.isLoadingContent = true
            editorState.loadFailed = null
            ...
            runCatching {
                ...
                if (!editorState.contentLoaded.isCompleted) {
                    editorState.contentLoaded.complete(Unit)
                }

                if (Settings.detect_bin_files && hasBinaryChars(loadedContent.toString())) {
                    editorState.editable = false
                    showNotice(BINARY_NOTICE_KEY) { id -> BinaryNotice(id) }
                }
            }.onFailure {
                editorState.loadFailed = it.message ?: strings.failed.getString()
                errorDialog(it)
            }
    ...
    fun queueSessionRestore(
        cursor: EditorCursorState,
        scrollX: Int,
        scrollY: Int,
        unsavedContent: String?,
    ) {
        pendingSessionRestore = PendingSessionRestore(cursor, scrollX, scrollY, unsavedContent)
    }

    private fun applyPendingSessionRestoreIfNeeded() {
        val pending = pendingSessionRestore ?: return
        ...
        restoreJob = scope.launch {
            ...
            withContext(Dispatchers.Main) {
                pending.unsavedContent?.let {
                    editorState.isDirty = true
                    editor.setText(it)
                }

                val maxLine = editor.text.lineCount - 1
                val lineLeft = pending.cursor.lineLeft.coerceAtMost(maxLine)
                val lineRight = pending.cursor.lineRight.coerceAtMost(maxLine)

                val maxColumnLeft = editor.text.getColumnCount(lineLeft)
                val maxColumnRight = editor.text.getColumnCount(lineRight)
                val columnLeft = pending.cursor.columnLeft.coerceAtMost(maxColumnLeft)
                val columnRight = pending.cursor.columnRight.coerceAtMost(maxColumnRight)

                editor.setSelectionRegion(lineLeft, columnLeft, lineRight, columnRight)
                editor.scroller.startScroll(pending.scrollX, pending.scrollY, 0, 0)
            }
        ...
        key(refreshKey) {
            ...
            Column {
                ...
                Column(modifier = Modifier.animateContentSize()) {
                    ...
                    AnimatedVisibility(
                        visible = editorState.isLoadingContent || editorState.isWrapping || editorState.isConnectingLsp,
                    ) {
