...
class FileTreeTab(val root: FileObject) : DrawerTab() {
    ...
    override fun Content(modifier: Modifier) {
        ...
        FileTree(
            ...
            headerActions = {
                if (HSPatcherWorkspaceIntegration.isHSPatcherWorkspace(root)) {
                    IconButton(
                        onClick = {
                            val command = HSPatcherWorkspaceIntegration.createBuildCommand(
                                java.io.File(root.getAbsolutePath()),
                            )
                            launchTerminal(context, command)
                        },
                    ) {
                        Icon(Icons.Outlined.Build, contentDescription = "Build HSPatcher workspace")
                    }
                }
            },
        )
