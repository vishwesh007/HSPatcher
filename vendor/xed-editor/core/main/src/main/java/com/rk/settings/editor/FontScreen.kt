...
fun rememberAppTypography(context: Context): Typography {
    ...
    val preferredFont =
        if (fontPath.isNotEmpty()) {
            FontCache.peekFont(fontPath)
        } else {
            null
        }
    val fallbackFont = FontCache.peekFont(DEFAULT_APP_FONT_PATH)
    val family =
        when {
            preferredFont != null -> FontFamily(preferredFont)
            fallbackFont != null -> FontFamily(fallbackFont)
            else -> null
        }
    return family?.let(::generateTypography) ?: com.rk.theme.Typography
}
