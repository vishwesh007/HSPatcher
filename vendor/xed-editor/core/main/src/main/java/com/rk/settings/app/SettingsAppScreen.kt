...
fun SettingsAppScreen(activity: SettingsActivity, navController: NavController) {
    PreferenceLayout(label = stringResource(id = strings.app), backArrowVisible = true) {
        ...
        val showOrientationSheet = remember { mutableStateOf(false) }
        val orientationLabels = listOf(stringResource(strings.orientation_portrait), stringResource(strings.orientation_landscape))
        val orientationModes = listOf(AppOrientation.MODE_PORTRAIT, AppOrientation.MODE_LANDSCAPE)

        PreferenceGroup {
            SettingsToggle(
                label = stringResource(strings.orientation_mode),
                description = when (Settings.app_orientation_mode) {
                    AppOrientation.MODE_LANDSCAPE -> stringResource(strings.orientation_landscape_desc)
                    else -> stringResource(strings.orientation_portrait_desc)
                },
                showSwitch = false,
                default = false,
                endWidget = {
                ...
                sideEffect = { showOrientationSheet.value = true },
            )
        ...
        if (showOrientationSheet.value) {
            ModalBottomSheet(
                onDismissRequest = { showOrientationSheet.value = false },
            ) {
                LazyColumn(modifier = Modifier.padding(bottom = 16.dp)) {
                    items(orientationModes.size) { index ->
                        val mode = orientationModes[index]
                        PreferenceTemplate(
                            title = { Text(text = orientationLabels[index]) },
                            description = {
                                Text(
                                    text = if (mode == AppOrientation.MODE_LANDSCAPE) {
                                        stringResource(strings.orientation_landscape_desc)
                                    } else {
                                        stringResource(strings.orientation_portrait_desc)
                                    },
                                )
                            },
                            modifier = Modifier.clickable {
                                Settings.app_orientation_mode = mode
                                AppOrientation.apply(activity)
                                showOrientationSheet.value = false
                            },
                            startWidget = {
                                RadioButton(selected = Settings.app_orientation_mode == mode, onClick = null)
                            },
                        )
                    }
                }
